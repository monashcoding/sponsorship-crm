...
api.post(
	...
	asyncHandler(async (req, res) => {
		...
		const result = await createCompany(db, {
			...body,
			createdBy: claimsOf(req).macUserId,
		});
		if ("duplicates" in result) {
			// Soft warning — the client resubmits with confirm:true to override.
			return res
				.status(409)
				.json({ error: "possible_duplicate", duplicates: result.duplicates });
		}
...
api.post(
	...
	asyncHandler(async (req, res) => {
		...
		const result = await reassign(db, {
			from,
			to,
			by: claimsOf(req).macUserId,
		});
