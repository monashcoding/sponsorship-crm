...
export const api = {
  ...
  patchContact: (id: string, body: Record<string, unknown>) =>
    apiJson<Contact>(`/api/contacts/${id}`, { method: "PATCH", body: JSON.stringify(body) }),

  deleteContact: (id: string) => apiJson<null>(`/api/contacts/${id}`, { method: "DELETE" }),
  ...
  reassign: (from: string, to: string) =>
    apiJson<{ companiesMoved: number; touchpointsMoved: number }>("/api/reassign", {
      method: "POST",
