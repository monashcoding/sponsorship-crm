...
import { fmtDate, fmtDateTime, isOverdue, localInputToISO } from "../lib/dates.js";
import {
  STAGES,
  STAGE_LABELS,
  STATUSES,
...
export function CompanyPage() {
  ...
  const detail = useQuery({ queryKey: ["company", id], queryFn: () => api.company(id) });
...
function CompanyFields({
  ...
  const save = useMutation({
    mutationFn: () => api.patchCompany(id, { website: w || null, industry: ind || null, notes: n || null }),
    onSuccess: onSaved,
  });
...
function StageStrip({ history }: { history: { id: string; fromStage: Stage | null; toStage: Stage; changedAt: string }[] }) {
...
function Contacts({
  ...
  companyId: string;
  contacts: { id: string; name: string; email: string | null; role: string | null; linkedin: string | null }[];
  onChange: () => void;
}) {
  ...
  const add = useMutation({
    mutationFn: () => api.addContact(companyId, { name, email: email || null, role: role || null }),
    onSuccess: () => {
      setName("");
      setEmail("");
      setRole("");
  ...
  const del = useMutation({ mutationFn: (cid: string) => api.deleteContact(cid), onSuccess: onChange });
      ...
      <ul className="contact-list">
        {contacts.map((c) => (
          <li key={c.id}>
            ...
            <button type="button" className="btn btn-ghost" onClick={() => del.mutate(c.id)}>
              Delete
            </button>
          </li>
        ))}
...
function AddTouchpoint({ companyId, onAdded }: { companyId: string; onAdded: () => void }) {
...
function Timeline({ touchpoints, onChange }: { touchpoints: Touchpoint[]; onChange: () => void }) {
  return (
    <ul className="timeline">
      {touchpoints.map((tp) => (
        <TimelineItem key={tp.id} tp={tp} onChange={onChange} />
      ))}
      {touchpoints.length === 0 && <li className="muted">No touchpoints yet.</li>}
    </ul>
  );
}

function TimelineItem({ tp, onChange }: { tp: Touchpoint; onChange: () => void }) {
  ...
  return (
    <li className="timeline-item">
      <div className="timeline-main">
        <span className={`status status-${tp.currentStatus ?? "sent"}`}>{tp.currentStatus}</span>
      ...
      <div className="timeline-controls">
        ...
        <label className="followup">
          ...
          <input
            type="datetime-local"
            defaultValue={tp.nextFollowUpAt ? tp.nextFollowUpAt.slice(0, 16) : ""}
            onChange={(e) => setFollowUp.mutate(e.target.value ? localInputToISO(e.target.value) : null)}
          />
          {isOverdue(tp.nextFollowUpAt) && <span className="flag" title="Overdue" />}
        </label>
      </div>
    </li>
  );
